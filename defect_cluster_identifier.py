...
def calc_severity_cluster(cluster, thresh):
    ...
    cluster_length = cluster[num_defects - 1][2] - cluster[0][2]  # Length is distance of last - first
...
def calc_severity_clusters(clusters, thresh, max_cluster_len):  # Calculate max, min, and avg severity value for clusters of different lengths
    ...
    for cluster_length in range(3, max_cluster_len + 1):
        ...
        filtered_clusters = [cluster for cluster in clusters if len(cluster) == cluster_length]
